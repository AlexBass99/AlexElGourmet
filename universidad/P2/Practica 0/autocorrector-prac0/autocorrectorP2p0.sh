...
MAXTIMEBIG=200 	   # Tiempo máximo de ejecución (en segundos) de una prueba lenta
...
# en esta p0, cada prueba es un programa que se compila con el fichero prac0.cc; Si no compila o no funciona, la prueba está mal
...
for i in $(ls $ENT/*.cc) ; do
 nf=$(basename $i .cc)
 echo -n "$nf "
 g++ -Wall -Wno-sign-compare -g $fuente $i -o $nf 2>&1
done
...
for i in $(ls $ENT/*.cc) ; do
  ...
  if test -s $nf ; then
	...
	if test "$nf" == "mainEj04"; then
	  # mas tiempo para la prueba 4
          $mata $MAXTIMEBIG $OBT/$nf.salida $VAL ./$nf > $OBT/$nf.salida 2> $tempfile
	else
